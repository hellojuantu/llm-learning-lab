# micrograd Course Design

这份文档记录 micrograd 课程怎么拆、每节课要练什么、怎么检查学生是否真的会了。
...
每节课都要让学习者在 notebook 里写、跑、改、查错，而不是只读解释。

## 不是题量，是理解台阶

题数可以变，但每节必须有一条清楚的理解台阶：
...
review_prompt.md 学完后用来复盘，不直接给答案
defense.md       项目答辩清单
```
...
课程改造完成后，不用“看起来更详细”作为标准，也不能只看脚本是否通过。自动检查只能说明格式、输出、检查器没有明显回退；它不能判断学生读起来是否顺、题目是否真的讲清楚。
...
第一屏是不是知道要做什么？
每个 TODO 是否知道要填什么？
qa_check 失败后能不能知道错在哪里？
提示是否比“看上面的例子”更具体？
答案是否藏在展开块里，而不是直接贴在题目下面？
上一题和下一题之间是否真的有复用关系？
```
